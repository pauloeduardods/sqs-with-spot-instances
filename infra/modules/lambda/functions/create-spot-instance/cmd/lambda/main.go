...
import (
	"context"
	"create-spot-instance/internal/ec2"
...
func main() {
	...
	ec2s, err := ec2.NewEC2Service()
	if err != nil {
		logger.Error("Error creating EC2 service: %v", err)
		return
	}

	ec2s.CreateSpotInstance()
	// req, err := ec2s.CheckForExistingSpotInstanceRequest()
	// logger.Info("req: %v", req)
	instances, err := ec2s.CheckForExistingSpotInstance()
	logger.Info("instance: %v", instances)
